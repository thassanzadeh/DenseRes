"""
Run a genetic algorithm to find an appropriate architecture for image segmentation

To use, define a `GenomeHandler` defined in genomehandler.py. Then pass it, with
training data, to a DEvol instance to run the genetic algorithm. 
"""
...
class DEvol:
    ...
    def __init__(self, genome_handler, data_path="SLIVER-1.csv"):
        """
    ...
    def run(self, dataset, num_generations, pop_size, epochs, fitness=None,
        ...
        pop = self._evaluate_population(members,
                                        ...
                                        0,
                                        num_generations)
        self.save_object(pop, 'initpop.pkl')  
        #with open('pop8.pkl', 'rb') as input:
        ...
        return load_model('best-modelg.h5')
    ...
    def _evaluate(self, genome, epochs):
        ...
        fit_params = {
            ...
            #'batch_size': 16,
            'callbacks': [
                EarlyStopping(monitor='val_loss', patience=3, verbose=1)#, ModelCheckpoint('/short/wm43/tk2020/weights-bestg.h5', save_weights_only=True, save_best_only=True)
            ]
        }
        ...
        try:
            ...
            seed = 2
            image_datagen.fit(self.x_train, seed=seed)
            mask_datagen.fit(self.y_train, seed=seed)
            image_generator = image_datagen.flow(self.x_train, batch_size=batch, seed=seed)
            mask_generator = mask_datagen.flow(self.y_train, batch_size=batch, seed=seed)
            train_generator = zip(image_generator, mask_generator)
            pl_model.fit_generator(train_generator, **fit_params)
            #
            ...
            loss, accuracy = self._handle_broken_model(pl_model, e)

        self._record_stats(pl_model, model, genome, loss, accuracy)

        return pl_model, loss, accuracy
    ...
    def _record_stats(self, pl_model, model, genome, loss, accuracy):
        ...
        if (self._bssf is -1 or
            ...
            try:
                ...
                os.remove('best-modelg.h5')
            except OSError:
                pass
            self._bssf = met
            model.save('best-modelg.h5')
    ...
    def _mutate(self, genome, generation):
        ...
        num_mutations = max(0, generation // 4)
